# avme-wallet-applications

Official repository with native DApps (Decentralized Applications) for the AVME Wallet.

## Documentation

### 2.0.0

* [Anatomy of a native DApp](docs/2.0.0/anatomy-of-a-dapp.md)
* [Test your DApp locally](docs/2.0.0/test-your-dapp-locally.md)
* [Submit or update your DApp](docs/2.0.0/submit-or-update-your-dapp.md)
* [Rules for submitting a DApp](docs/2.0.0/rules-for-submitting-a-dapp.md)
* [How the API works](docs/2.0.0/how-the-api-works.md)
* [API reference](docs/2.0.0/api-reference)

